...
encodings <- readr::guess_encoding("china_mapdata.csv") # to confirm the encoding types of your data
chinamap <- read_csv("china_mapdata.csv", locale = locale(encoding = encodings$encoding[1])) # import your data according to the encoding types
...
setwd("D:/Xiaoyu Zai/PhD research project/Domestication of foxtail millet/My Research Project/data")
...
dart.matrix.data <- read.csv("SNP analysis/snp_dis.csv", 
                      header = T, 
...
# whole ASVs
e19.mean.core <- t(read.csv("SNP analysis/ASV_table_bacteria.tsv",
                          header = T,
                          sep = "\t",
                          row.names = 1,check.names=F)) # 行为样本名

# core genera						  
e19.mean.core <- read.csv("SNP analysis/core bacterial genera.txt",
                            header = T,
                            sep = "\t",
                            row.names = 1,check.names=F) # 行为样本名 

# env factors
e19.mean.env <- read.csv("SNP analysis/env.txt",
                          header = T,
                          sep = "\t",
                          row.names = 1,check.names=F) # 行为样本名
...
# write the table
df2=cbind(result1$a,result2$a)
write.table(df2,"leaf_16s/env_explain.tsv",row.names=T,sep="\t")
